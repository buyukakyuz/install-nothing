...
impl InstallationStage for FilesystemStage {
    ...
    fn run(&self, exit_check: &dyn Fn() -> bool) -> io::Result<()> {
        ...
        println!("{}", format!("Creating filesystem with {} 4k blocks and {} inodes", blocks, inodes).dimmed());
        ...
        println!("{}", "Filesystem UUID: 8f3e1a2b-4c5d-6e7f-8a9b-0c1d2e3f4a5b".dimmed());
        println!("{}", format!("Superblock backups stored on blocks:").dimmed());
        ...
        progress.animate("Allocating group tables:", rng.gen_range(2000..3000), exit_check)?;

        let progress = ProgressBar::new(ProgressStyle::Equals);
        progress.animate("Writing inode tables:", rng.gen_range(2500..4000), exit_check)?;
        ...
        println!("{}", "Writing superblocks and filesystem accounting information: ".dimmed());
        ...
        if rng.gen_bool(0.4) {
            ...
            println!("{}", "/dev/sda2: 11/2048000 files (0.0% non-contiguous), 200000/8192000 blocks".bright_green());
        }
